...
public final class PersonaListener implements Listener {
    ...
    @EventHandler(ignoreCancelled=true) public void npcLeftClick(NPCLeftClickEvent e){var routed=plugin.behaviors().projections().routed(e.getNPC(),e.getClicker());if(routed==null){e.setCancelled(true);return;}plugin.behaviors().projections().markInteraction(e.getClicker(),routed);plugin.behaviors().wake(routed,e.getClicker(),"interaction",Map.of("button","left"));}
    ...
    @EventHandler(ignoreCancelled=true) public void npcDamage(NPCDamageByEntityEvent e){if(!(e.getDamager() instanceof org.bukkit.entity.Player player))return;var routed=plugin.behaviors().projections().routed(e.getNPC(),player);if(routed==null){e.setCancelled(true);return;}plugin.behaviors().wake(routed,player,"damage",Map.of("amount",e.getDamage()));}
    @EventHandler public void npcSpawn(NPCSpawnEvent e){PersonaTrait t=e.getNPC().getTraitNullable(PersonaTrait.class);if(t!=null&&t.bound()&&!t.projection())plugin.behaviors().actorLifecycle(e.getNPC(),"spawn",Map.of());}
    @EventHandler public void npcDespawn(NPCDespawnEvent e){plugin.dialogues().cancelNpc(e.getNPC());PersonaTrait t=e.getNPC().getTraitNullable(PersonaTrait.class);if(t!=null&&t.bound()&&!t.projection())plugin.behaviors().actorLifecycle(e.getNPC(),"despawn",Map.of("reason",e.getReason().name()));}

...
public final class PersonaListener implements Listener {
    ...
    @EventHandler public void join(PlayerJoinEvent e){plugin.states().load(e.getPlayer());}
    @EventHandler public void quit(PlayerQuitEvent e){plugin.dialogues().cancel(e.getPlayer().getUniqueId(),null);plugin.quests().tickPlayer(e.getPlayer(),false);plugin.states().unload(e.getPlayer());lastMove.remove(e.getPlayer().getUniqueId());}
    ...
    @EventHandler(ignoreCancelled=true) public void npcClick(NPCRightClickEvent e){plugin.dialogues().begin(e.getClicker(),e.getNPC());}

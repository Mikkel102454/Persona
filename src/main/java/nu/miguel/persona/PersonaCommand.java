...
public final class PersonaCommand implements CommandExecutor,TabCompleter {
    ...
    private void editor(CommandSender sender,String[] args){
        ...
        if(args.length>1&&Set.of("sessions","trust","revoke","close","rollback").contains(args[1].toLowerCase(Locale.ROOT))){editorSession(sender,args);return;}
        EditorScope scope;
    ...
    private void editorSession(CommandSender sender,String[] args){
        ...
        if(args.length<3){sender.sendMessage(Component.text("/persona editor <trust|revoke|close> <session-id> | rollback <session-id> <publish-id> confirm"));return;}

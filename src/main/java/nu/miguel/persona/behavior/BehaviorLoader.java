...
public final class BehaviorLoader {
    ...
    private static void validateExtension(Map<String,Object> schema,ConfigurationSection s,Set<String> allowed){if(schema==null||schema.isEmpty())return;Object properties=schema.get("properties");if(properties instanceof Map<?,?> p)for(Object key:p.keySet())allowed.add(String.valueOf(key));Map<String,Object> options=new LinkedHashMap<>();for(String key:s.getKeys(false))if(!BASE.contains(key)&&!Set.of("action","condition").contains(key))options.put(key,s.get(key));Validation.schema(schema,options,"extension node");}

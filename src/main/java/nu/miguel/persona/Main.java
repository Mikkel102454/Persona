...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        states=new StateManager(this,store);effects=new EffectExecutor(this);quests=new QuestService(this);scripts=new ScriptEngine(this);dialogues=new DialogueService(this);
    ...
    public boolean reloadPersona(){if(dialogues!=null)dialogues.cancelAll("Conversation cancelled by content reload.");reloadConfig();try{registry.loadAndReplace(this::loadContent);return true;}catch(ContentException e){e.errors().forEach(x->getLogger().severe("Persona reload rejected: "+x));return false;}catch(RuntimeException e){getLogger().severe("Persona reload rejected: "+e.getMessage());return false;}}
